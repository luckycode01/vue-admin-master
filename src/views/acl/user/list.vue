<template>
  <div class="app-container">
    ...
    <div style="margin-bottom: 20px">
      <el-button type="primary" @click="showAddUser"  v-if="$hasBtnPower('btn.User.add')">添 加</el-button>
      <el-button type="danger" @click="revomveUsers" :disabled="selectedIds.length===0"
      v-if="$hasBtnPower('btn.User.remove')"
        >批量删除</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="{row}">
          <HintButton type="info" size="mini" icon="el-icon-user-solid" title="分配角色"  v-if="$hasBtnPower('btn.User.assgin')"
            @click="showAssignRole(row)"/>
          <HintButton type="primary" size="mini" icon="el-icon-edit" title="修改用户" v-if="$hasBtnPower('btn.User.update')"
            @click="showUpdateUser(row)"/>
          <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
            <HintButton style="margin-left:10px" slot="reference" type="danger" size="mini" icon="el-icon-delete" v-if="$hasBtnPower('btn.User.remove')"
              title="删除用户"/>
          </el-popconfirm>
        </template>
      </el-table-column>
    ...
    <el-dialog title="设置角色" :visible.sync="dialogRoleVisible" :before-close="resetRoleData">
      <el-form label-width="80px">
        ...
        <el-form-item label="角色列表">
          ...
          <!-- 多选框组用法和select很像
          它的v-model收集的是选中的checkbox的lable值
